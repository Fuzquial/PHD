...
# User parameters
# ============================================================
MODEL_NAME = 'Model-5MPa'
JOB_NAME   = '5MPa_hetero_test'
...
def weighted_quantile_thresholds(values, weights, fracs):
    f1 = fracs[0]
    f2 = fracs[0] + fracs[1]
    pairs = [(values[i], weights[i]) for i in range(len(values))]
...
mdl.HomogeneousSolidSection(name='Section-Feldspath', material='Feldspath',        thickness=None)
mdl.HomogeneousSolidSection(name='Section-Quartz',    material='Quartz',           thickness=None)
mdl.HomogeneousSolidSection(name='Section-Biotite',   material='Biotite',          thickness=None)
...
print("Modele construit: %s" % MODEL_NAME)
