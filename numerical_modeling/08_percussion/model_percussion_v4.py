...
minerals = [
    dict(name='Feldspath', E=48000.0, nu=0.25, rho=2.1e-9,
         ...
         fail=dict(e_crack_dir=0.005, e_crack_ind=0.01, fc_ratio=0.86, fbc_ratio=0.83)),
...
for m in minerals:
    ...
    # Compression damage : dc_max calcule pour garantir eps_pl > 0
    # Formule : dc/(1-dc) < EIN_END * E / cres  =>  dc < ratio/(1+ratio)
    if cres > 1e-10:
        ratio_dc = EIN_END * m['E'] / cres
        dc_max = min(0.95, ratio_dc / (1.0 + ratio_dc) * 0.9)  # marge de securite 10%
    else:
        dc_max = 0.95
    mat.concreteDamagedPlasticity.ConcreteCompressionDamage(
        table=((0.0, 0.0), (dc_max, EIN_END))
    )
...
# ============================================================
# 15. CREATION DU JOB + *Concrete Failure (element deletion)
# ============================================================
# *Concrete Failure, TYPE=Strain n'a pas d'API Python -> on edite le .inp
...
# Ecrire le fichier input
mdb.jobs[JOB_NAME].writeInput(consistencyChecking=OFF)

# Editer le .inp : inserer *Concrete Failure apres chaque *Concrete Tension Damage
...
while i < len(lines):
    ...
    # Detecter une ligne *Concrete Tension Damage
    if lines[i].strip().upper().startswith('*CONCRETE TENSION DAMAGE'):
        # Copier les lignes de donnees qui suivent (jusqu'au prochain *)
        i += 1
        while i < len(lines) and not lines[i].strip().startswith('*'):
            new_lines.append(lines[i])
            i += 1

        # Determiner quel mineral correspond
        # Remonter pour trouver le *Material, name=XXX
        mat_name = ''
        for j in range(len(new_lines) - 1, -1, -1):
            if new_lines[j].strip().upper().startswith('*MATERIAL'):
                mat_name = new_lines[j].split('name=')[-1].strip().rstrip('\n').rstrip('\r')
                break

        # Chercher les parametres de failure du mineral
        fail_params = None
        for m in minerals:
            if m['name'].upper() == mat_name.upper():
                fail_params = m.get('fail', None)
                break

        if fail_params is not None:
            new_lines.append('*Concrete Failure, TYPE=Strain\n')
            new_lines.append('%.8g, %.8g, %.4g, %.4g\n' % (
                fail_params['e_crack_dir'],
                fail_params['e_crack_ind'],
                fail_params['fc_ratio'],
                fail_params['fbc_ratio']
            ))
            print("  *Concrete Failure insere pour %s" % mat_name)

        continue   # ne pas incrementer i, deja positionne
...
print("Fichier %s ecrit avec *Concrete Failure, TYPE=Strain." % inp_path)
print("Pour lancer : abaqus job=%s input=%s cpus=14" % (JOB_NAME, inp_path))
...
print("  Element deletion : *Concrete Failure, TYPE=Strain")
print("  Lancer via : abaqus job=%s input=%s.inp cpus=14" % (JOB_NAME, JOB_NAME))
print("=" * 60)