...
# --- Maillage ---
ELEMENT_SIZE_FINE   = 0.3    # zone d'impact (0.2 -> 0.3 : ~3.4x moins d'elements)
ELEMENT_SIZE_COARSE = 2.0    # zone eloignee
ELEM_CODE_HEX = C3D8R
ELEM_CODE_TET = C3D4
...
CDP_FBFC  = 1.16
CDP_K     = 0.554
CDP_VISC  = 1e-3      # augmente pour convergence implicite (etait 5e-4)
...
# --- Step dynamique ---
STEP_TIME    = 0.001      # s
STEP_DT_INIT = 5e-6       # s (demarre plus petit pour le contact initial)
STEP_DT_MIN  = 1e-10      # s
STEP_DT_MAX  = 2e-5       # s (plus grand = moins d'increments)
STEP_MAX_INC = 50000      # plus de marge pour converger
...
# 9. CONTACT
# ============================================================
# Note : pour une part ANALYTIC_RIGID_SURFACE avec un RP,
# Abaqus cree le rigid body automatiquement. Pas besoin de mdl.RigidBody().

# Propriete de contact : sans frottement + Augmented Lagrange
...
mdl.SurfaceToSurfaceContactStd(
    ...
    main=inst_ind.surfaces['SURF_ANALYTICAL'],
    secondary=aAsm.surfaces['SURF_TOP'],
    sliding=FINITE,
    thickness=ON,
...
print("Contact pair cree.")

# ============================================================
# 10. STEP DYNAMIQUE
# ============================================================
mdl.ImplicitDynamicsStep(
    name='Step-1',
    previous='Initial',
    nlgeom=ON,
    timePeriod=STEP_TIME,
    maxNumInc=STEP_MAX_INC,
    initialInc=STEP_DT_INIT,
    minInc=STEP_DT_MIN,
    maxInc=STEP_DT_MAX,
    application=DEFAULT
)
print("Step dynamique implicite cree (T=%.4f s, inertie reelle)." % STEP_TIME)
...
mdl.FieldOutputRequest(
    name='F-Output-1',
    createStepName='Step-1',
    variables=('S', 'LE', 'PE', 'PEEQ', 'MISES', 'U', 'V', 'A', 'RF'),
    frequency=20
)
...
print("Output requests crees (field: freq=20, history: freq=5).")
