...
# ============================================================
# 5. SECTIONS + MAILLAGE + HETEROGENEITE
# ============================================================
...
box.seedPart(size=ELEMENT_SIZE, deviationFactor=0.1, minSizeFactor=0.1)
box.setMeshControls(regions=box.cells, technique=SWEEP, algorithm=MEDIAL_AXIS)
...
# ============================================================
IND_REF   = 8.0
IND_SCALE = min(BOX_LX, BOX_LY) / 2.0 / IND_REF
...
# Contact pair : surface analytique (main) vs face top echantillon (secondary)
...
print("Step dynamique implicite cree (T=%.4f s)." % STEP_TIME)
...
for key in mdl.fieldOutputRequests.keys():
    if key != 'F-Output-1':
        safe_delete(mdl.fieldOutputRequests, key)
for key in mdl.historyOutputRequests.keys():
    safe_delete(mdl.historyOutputRequests, key)

mdl.FieldOutputRequest(
    name='F-Output-1',
    createStepName='Step-1',
    variables=('S', 'E', 'LE', 'PE', 'PEEQ', 'MISES', 'EVOL',
               'U', 'V', 'A', 'RF', 'CF', 'PEMAG', 'PEEQT'),
    frequency=1
)
